...
if(isset($_GET["do"]) && $_GET["do"]=="guardar"){
  ...
  $lfechaapli= $_POST["txtfecha"];
  
  ActuaPrecioFuturoMatriz($lruta,$lfechaapli,$lmatriz);
 }

$dat=ListaPrecios($lruta,$lproducto);
$lafecha='1900-01-01';
for($x=0;$x<count($dat);$x++)
  if(substr($dat[$x]["fechaapli"],0,10)>$lafecha)
    $lafecha=substr($dat[$x]["fechaapli"],0,10);
...
<head>
  ...
  <link rel="stylesheet" href="css/precios.css" type="text/css" />
    ...
    <script type="text/javascript" src="js/preciosfuturos.js"></script>
...
<form name='formsegu' method='post' action='preciosfuturos.php'>
<h1>Precios futuros</h1>
 <div id='controles'>
 ...
 <div>
  ...
  <div id='detalle'>
    <div style='margin-bottom:10px;'>
      Aplicar en esta fecha: <input type='date' id='txtfecha' name='txtfecha' value="<?php  echo $lafecha; ?>">
    </div>
    <table id='tablamarca'>
       <thead>
       <tr>
       <th>Parada abordaje</th><th></th><th>Parada llegada</th><th>Precio Actual</th><th>Precio Nuevo</th>
       </tr>
       </thead>
       <tbody>
       <?php
        ...
        for($x=0;$x<count($dat);$x++){
           ...
           echo "<td>".$dat[$x]["precio"]."</td>";
           echo "<td><input type='number' step='any' id='txtprecio' style='text-align:right;' value='".$dat[$x]["preciofuturo"]."'></td>";
           echo "</tr>";
        }
