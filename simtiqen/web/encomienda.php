...
if($this->cliente->idCliente==1 && strlen($this->cedremi)>0)
        ...
        if($this->idEncomienda==0) {
         ...
         $sql =sprintf("insert into encomienda (fecha,idviaje,idestacion,
          ...
          nota,cantbul,declarado,fechadi,monto,idimpresora,esmanual,idautoriza,autocedula,autonombre,retirada,idtipotrack,idformapago
          ,fecharetiro,estaretiro,entcierre,entimpresora,entusuario)
          value ('%s',%d,%d,%d,%d,%d,%d,%d,%d,'%s',
          %d,'%s','%s','%s','%s','%s','%s','%s','%s','%s',%d,%d,'%s',%d,%d,%d,%d,'%s','%s',%d,%d,%d,'1900-01-01',%d,%d,%d,%d)",
          $this->fecha,
          $this->viaje->idViaje,
...
$this->bitacora->Bitacorizar("Guarda Firma");
