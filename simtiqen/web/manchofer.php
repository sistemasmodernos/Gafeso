...
<h2 style='margin:auto;width:400px;'>Mantenimiento de Choferes<h2>
...
if(isset($_POST['submit'])){
   ...
   $consulta = ListaMantenimiento("chofer",$elfiltro,array("pagina"=>$pagina,"registros"=>$limite));
  	$registros = Paginas("chofer",$elfiltro);
   $xbus = $_POST['buscar'];
}else{
   $consulta = ListaMantenimiento("chofer",null,array("pagina"=>$pagina,"registros"=>$limite));
  	$registros = Paginas("chofer",null);
   $xbus = '';
}
?>
...
<form id="frmConsulta" name="frmConsulta" method="post" action="" onSubmit="GrabarDatos(); return false">
    ...
    <label>Incluir un nuevo registro</label>
          <span id="nuevo"><a href="newchofer.php"><img src="img/add.png" title="Agrega un nuevo registro" alt="Agregar dato" /></a></span>
</form>
</div>
<form id="frmDatos">
	<table>
...
if($consulta) {
	...
	$linea = $consulta[$i];
	?>
		  <tr id="fila-<?php echo $linea['idchofer'] ?>">
			  <td><?php echo $linea['Nombre'] ?></td>
			  <td><?php echo $linea['cedula'] ?></td>
			  <td><?php echo $linea['correo'] ?></td>
			  <td><span class="modi"><a href="actchofer.php?idchofer=<?php echo $linea['idchofer'] ?>"><img src="img/database_edit.png" title="Editar el registro" alt="Editar" /></a></span></td>
			  <td><span class="dele"><a onClick="EliminarDato(<?php echo $linea['idchofer'] ?>); return false" href="elichofer.php?idchofer=<?php echo $linea['idchofer'] ?>"><img src="img/delete.png" title="Elimina el registro seleccionado" alt="Eliminar" /></a></span></td>
		  </tr>
	<?php
	}
}
