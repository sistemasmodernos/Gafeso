...
<form name='formlasiento' method='post' action='lretiraenco.php'>
<div id='controles'>
...
<div id='divdatos'>
<div id='divtitulo'>
      <p> Listado de retiro de encomiendas <br/> Impresora 
      <?php 
       if($limpresora!=''){
    ...
    <?php
       if($limpresora!='')
        $dat= ListaActivos("Encomienda",array(
                  "e.fecharetiro between '".fechatoUTC($lfechai)."' and '".fechatoUTC($lfechaf)."' ",
        ...
        $dat = ordenaMatriz($dat,"fecharetiro");
        $maxpag=30; //cantidad de líneas que caben en una página
       $cont=1;
       $enca = "<div class='divpag'><table class='tabasientos'>";
       $enca .="<thead>
            <tr>
       ...
       for($x=0;$x<count($dat);$x++){
           if($cont>=$maxpag)
           {
            ...
            "</td><td>".trim($dat[$x]["autocedula"])." ".$dat[$x]["autonombre"];
              $total+=$dat[$x]["monto"];
           echo "</td></tr>";
           
           $cont++;
       }
              echo "<tfoot><tr><th colspan='6'>Total</th><th>".number_format($total,2)."</th><th></th></tr></tfoot>";
       echo "</table></div>";
    ?>
</div>
</div>
