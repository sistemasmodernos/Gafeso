...
<h2 style='margin:auto;width:400px;'>Mantenimiento de Estaciones<h2>
...
if(isset($_POST['submit'])){
	$elfiltro = array();
	array_push($elfiltro,"nombre like '%".$_POST['buscar']."%'"); 
   $consulta = ListaMantenimiento("estacion",$elfiltro,array("pagina"=>$pagina,"registros"=>$limite));
  	$registros = Paginas("estacion",$elfiltro);
   $xbus = $_POST['buscar'];
}else{
   $consulta = ListaMantenimiento("estacion",null,array("pagina"=>$pagina,"registros"=>$limite));
  	$registros = Paginas("estacion",null);
   $xbus = '';
}
?>
...
<form id="frmConsulta" name="frmConsulta" method="post" action="" onSubmit="GrabarDatos(); return false">
    ...
    <label>Incluir un nuevo registro</label>
          <span id="nuevo"><a href="newestacion.php"><img src="img/add.png" title="Agrega un nuevo registro" alt="Agregar dato" /></a></span></form>
</div><form id="frmDatos">
	<table>
   		<tr>
   			<th>Nombre de la Estaci&oacute;n</th>
		    <th></th>
            <th></th>
        </tr>
<?php
if($consulta) {
	...
	$linea = $consulta[$i];
	?>
		  <tr id="fila-<?php echo $linea['idestacion'] ?>">
			  <td><?php echo $linea['nombre'] ?></td>
			  <td><span class="modi"><a href="actestacion.php?idestacion=<?php echo $linea['idestacion'] ?>"><img src="img/database_edit.png" title="Editar el registro" alt="Editar" /></a></span></td>
			  <td><span class="dele"><a onClick="EliminarDato(<?php echo $linea['idestacion'] ?>); return false" href="eliestacion.php?idestacion=<?php echo $linea['idestacion'] ?>"><img src="img/delete.png" title="Elimina el registro seleccionado" alt="Eliminar" /></a></span></td>
		  </tr>
	<?php
	}
}
