    	...
    	switch($opcion) {
   		/* 1: Agregar un registo
    		...
    		case 1: $retorno = $ob->Guardar(null) > 0;
    		        break;
    		case 2: $retorno = $ob->Guardar(null) > 0;
    		        break;
...
function CreaCierre($pestacion,$pimpresora,$fechai,$fechaf,$pingresos,$pegresos){
   $cie = new Cierred();
   $cie->estacion->idEstacion=$pestacion;
   $cie->impresora->idImpresora = $pimpresora;
   $cie->fechai = $fechai;
...
function Asientosxnum($pviaje){
    ...
    $tiq = new Tiquete();
    $res = $tiq->ListarActivos(array("t.idviaje=".$pviaje,"t.estado=0"));
    $bol = ResultArray($res);
...
function AsientosxWeb($pviaje){
    ...
    //        "idestacion"=>0));
    $tiq = new Tiquete();
    $res = $tiq->ListarActivos(array("t.idviaje=".$pviaje,"t.estado=0","t.idestacion=4"));
    $bol = ResultArray($res);
    //    $asi[0]["parada"]="y De pie";
    for($x=0;$x<count($bol);$x++){
        if($bol[$x] ["asiento"]<1){
...
function ClonaViajes($desde,$hasta){
	...
	//			           echo "entro a revisar <br/>";
					$xres=$oviaje->Listar(array(" idruta=".$ruta, " fecha='".date('Y-m-d',$date)."'"," hora='".$lviaje. "'"," v.idtipoviaje='".$lviaje["tipo"]. "'"), null);
...
function cambiahora($pimpresora,$pfactura,$pidviaje,$pparada)
{
    $tiqv = new Tiquete();
 ...
 //   echo "va a cargar el  viaje ".$pidviaje."<br/>";
    ...
    $detbit="Cambio de horario en tiquete ".$tiqn->factura." Horario anterior:  ".$tiqv->viaje->fecha ." " .$tiqv->viaje->hora.
    " Parada anterior ".$tiqv->parada1->nombre."  Horario nuevo :".
    $tiqn->viaje->fecha ." " .$tiqn->viaje->hora.
...
function ImprimeTiq2Standard($ptiquetes){
  $tiqs=explode(",",$ptiquetes);
  $cad="";
  for($x=0;$x<count($tiqs);$x++){
    $cad.= "<div class='divtiq'>".
       str_replace("\n","<br/>",ImprimeTiqueteStandard($tiqs[$x])).
       "</div> ";
  }
  return $cad;
}
...
function RetiraEncomienda($pidencomiendas,$pCombo,$pCedula,$pNombre){
    $enco = new Encomienda();
