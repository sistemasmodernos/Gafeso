...
<h2 style='margin:auto;width:400px;'>Mantenimiento de Buses<h2>
...
if(isset($_POST['submit'])){
	$elfiltro = array();
	array_push($elfiltro,"placa like '%".$_POST['buscar']."%' or color like '%".$_POST['buscar']."%'"); 
   $consulta = ListaMantenimiento("bus",$elfiltro,array("pagina"=>$pagina,"registros"=>$limite));
  	$registros = Paginas("bus",$elfiltro);
   $xbus = $_POST['buscar'];
}else{
   $consulta = ListaMantenimiento("bus",null,array("pagina"=>$pagina,"registros"=>$limite));
  	$registros = Paginas("bus",null);
   $xbus = '';
}
?>
...
<form id="frmConsulta" name="frmConsulta" method="post" action="" onSubmit="GrabarDatos(); return false">
    ...
    <label>Incluir un nuevo registro</label>
          <span id="nuevo"><a href="newbus.php"><img src="img/add.png" title="Agrega un nuevo registro" alt="Agregar dato" /></a></span>
...
if($consulta) {
	...
	$linea = $consulta[$i];
	?>
		  <tr id="fila-<?php echo $linea['idBus'] ?>">
			  <td><?php echo $linea['placa'] ?></td>
			  <td><?php echo $linea['color'] ?></td>
			  <td><?php echo $linea['cantpas'] ?></td>
			  <td><?php echo $linea['chofer'] ?></td>
        <td><?php echo $linea['socio'] ?></td>
        <td><?php echo $linea['asientosespe'] ?></td>
			  <td><span class="modi"><a href="actbus.php?idbus=<?php echo $linea['idBus'] ?>"><img src="img/database_edit.png" title="Editar el registro" alt="Editar" /></a></span></td>
			  <td><span class="dele"><a onClick="EliminarDato(<?php echo $linea['idBus'] ?>); return false" href="elibus.php?idBus=<?php echo $linea['idBus'] ?>"><img src="img/delete.png" title="Elimina el registro seleccionado" alt="Eliminar" /></a></span></td>
		  </tr>
	<?php
	}
}
