...
function generaventa(){
	    ...
	    if($("#tipocedfa").val() != '00'){
        var actividad = $("#txtactividad").val().trim();
        
        // Verificar que no esté vacío
        if(actividad == ""){
            alert("Debe ingresar el código de actividad económica.\n\nSi su cliente NO lo sabe, debe hacerle un TIQUETE seleccionando 'Impersonal'");
            $("#txtactividad").focus();
            return;
        }
        
        // Verificar formato xxxx.x
        if (!actividad.match(/^\d{4}\.\d{1}$/)) {
            alert("El formato del código de actividad no es válido.\n\nDebe ser: xxxx.x (ejemplo: 4923.0)");
            $("#txtactividad").focus();
            return;
        }
        
        // Verificar que sea válido según el span
        var spanTexto = $("#spanactividad").text();
        if(spanTexto.indexOf("❌") >= 0 || spanTexto == ""){
            alert("El código de actividad económica no es válido.\n\nVerifique que el código exista en el catálogo.");
            $("#txtactividad").focus();
            return;
        }
    }
  ...
  function cambiatipocedula(){
  	var tip = $("#tcedremi").val();
	    ...
	    $("#spanactividad").html("");
		  ...
		  $("#datosparafe").show();
		  loadDirecciones();
	    if($("#txtactividad").val().trim() != ""){
	    	validaActividad();
			} else {
		    $("#spanactividad").html("❌ Campo requerido").css("color", "orange");
	    }
		}
  }
...
// Función para validar la actividad económica
function validaActividad(){
    var codigo = $("#txtactividad").val().trim();
    ...
    if(!/^\d{4}\.\d{1}$/.test(codigo)){
        $("#spanactividad").html("❌ Formato debe ser xxxx.x").css("color", "red");
        return false;
    }
    
    // Consultar si existe en la BD
    $("#spanactividad").html("⏳ Verificando...").css("color", "blue");
    var rut = "enlace.php?opcion=validaactividad&codigo=" + codigo;
    enlace(rut, fin_validaActividad);
    return true;
}

function fin_validaActividad(dato){
    var datos = dato.split('|');
    
    if(datos[0] == "1"){
        // Existe - Mostrar descripción
        $("#spanactividad").html("✓ " + datos[1]).css("color", "green");
    } else {
        // No existe
        $("#spanactividad").html("❌ Código no válido").css("color", "red");
    }
}
