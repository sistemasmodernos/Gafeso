...
if(isset($_POST["cbestacion"]) && $_POST["cbestacion"]!=""){
  $filtros.=" and t.idestacion= ".$_POST["cbestacion"];    
...
<form name='formlasiento' method='post' action='ventagralcajero.php'>
<div id='controles'>
       Horarios desde: 
    ...
    <div class="filtro">
       <label for='cbestacion' >Estacion: </label>
       <select id='cbestacion' name='cbestacion' >
    <?php
...
<div id='divcuadro'>
    <?php
       ...
       $dat= VentaGralCajero(fechatoUTC($lfechai),fechatoUTC($lfechaf),$filtros) ;
       ...
       for($x=0;$x<count($dat);$x++){
          if($cajero != $dat[$x]["usuario"]){
            if ($cajcuenta > 0){
             echo "<tr><th colspan='3'>Total de ".$cajero."</th>";
             echo "<th>".number_format($cajcuenta,0)." boletos</th><th>".number_format($cajtotal,2)."</th>";
             echo "<th colspan='3'>Musoc ".number_format($cajcuemus,0)." - ¢ ".number_format($cajtotmus,2)."</th>";
             echo "<th colspan='3'>Santos ".number_format($cajcuesan,0)." - ¢ ".number_format($cajtotsan,2)."</th>";
             echo "</tr>";
            }
             $cajcuenta = 0;
             $cajtotal = 0;
         ...
         //     echo $cabezatabla;
           ...
           $cajtotal += $dat[$x]["monto"];
           $cajcuenta++;
    	   $cuenta++;
           $cont++;
           if ($dat[$x]["idruta"]<3){
                $cajcuemus++;
                $cajtotmus+=$dat[$x]["monto"];
                $totmus+=$dat[$x]["monto"];
                $cuemus++;
           }else{
                $cajcuesan++;
                $cajtotsan+=$dat[$x]["monto"];
                $totsan+=$dat[$x]["monto"];
                $cuesan++;
           }
       }
       if ($cajcuenta > 0){
            ...
            echo "<th>".number_format($cajcuenta,0)." boletos</th><th>".number_format($cajtotal,2)."</th>";
            echo "<th colspan='3'>Musoc ".number_format($cajcuemus,0)." - ¢ ".number_format($cajtotmus,2)."</th>";
            echo "<th colspan='3'>Santos ".number_format($cajcuesan,0)." - ¢ ".number_format($cajtotsan,2)."</th></tr>";
       }
       echo "<tr><th colspan='3'>Total</th>";
       echo "<th>".number_format($cuenta,0)." boletos</th><th>".number_format($total,2)."</th>";
       echo "<th colspan='3'>Musoc ".number_format($cuemus,0)." - ¢ ".number_format($totmus,2)."</th>";
       echo "<th colspan='3'>Santos ".number_format($cuesan,0)." - ¢ ".number_format($totsan,2)."</th>";
       echo "</tr>";
       echo "</table></div>";
    ?>
</div>
