...
class FacturaEnco 
{
	...
	function generafactura($pid){
		...
		if(substr($enco->facturafe, 8,2)=='01')
			$xml= $this->generaxmlFacturaV44($enco);
	    else
	    	$xml= $this->generaxmlTiqueteV44($enco);
	    echo $xml;
...
function generaxmlFacturaV44($pventa){
                ...
                $fecha =  $fechaactual->format('Y-m-d\TH:i:s').".999";
                //$fecha = str_replace(" ","T",$pventa->fechadi).".999";
                $monto = $pventa->monto-$pventa->iva;
                $iva = $pventa->iva;
                $total = $pventa->monto;
                $strimpu="";
		$strResumen="";
		$actividadreceptor = $pventa->actividad;
                $empre = new Empresa();
                if($pventa->tipocedfa=='00'){
                        $strimpu='
                        <Impuesto>
                        ...
                        <ResumenFactura>
                                ...
                                <TotalDesgloseImpuesto>
                                        <Codigo>01</Codigo>
                                        <CodigoTarifaIVA>08</CodigoTarifaIVA>
                                        <TotalMontoImpuesto>'.$iva.'</TotalMontoImpuesto>
                                </TotalDesgloseImpuesto>
                                <TotalImpuesto>'.$iva.'</TotalImpuesto>
                        <MedioPago>
                ...
                {
                        if($empre->CargarCedula($pventa->cedremi)>0)
                        {
                                $poriva = $empre->getPorIva();
                                if($poriva==0){
                                        $strimpu='';
                     ...
                     $impexo=round($brutoexo*$poriva/100,2);
                                                ...
                                                $strimpu='
                                                        <Impuesto>
                                                                <Codigo>01</Codigo>
                                                                <CodigoTarifaIVA>'.$empre->tarifaimp.'</CodigoTarifaIVA>
                                                                <Tarifa>'.$poriva.'</Tarifa>
                                                                <Monto>'.($impexo+$iva).'</Monto>
                                                                <Exoneracion>
                                                                        <TipoDocumento>03</TipoDocumento>
                ...
                xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                        <Clave>'.$clave.'</Clave>
                        <ProveedorSistemas>3102728401</ProveedorSistemas>
                        <CodigoActividadEmisor>'.$codact.'</CodigoActividadEmisor>                ';
                       if($actividadreceptor!='')
                          $xml.='<CodigoActividadReceptor>'.$actividadreceptor.'</CodigoActividadReceptor>';
                       $xml.='
                        <NumeroConsecutivo>'.$numfa.'</NumeroConsecutivo>
                        <FechaEmision>'.$fecha.'</FechaEmision>
                        <Emisor>
...
';
        ...
        $xml.='
                ...
                <DetalleServicio>
                   <LineaDetalle>
                     <NumeroLinea>1</NumeroLinea>
                     <CodigoCABYS>6511600000000</CodigoCABYS>
                     <Cantidad>1</Cantidad>
                         <UnidadMedida>Os</UnidadMedida>
                     <Detalle>Encomienda '.$pventa->nota.'</Detalle>
                     <PrecioUnitario>'.$monto.'</PrecioUnitario>
                     <MontoTotal>'.$monto.'</MontoTotal>
                         <SubTotal>'.$monto.'</SubTotal>
                         <BaseImponible>'.$monto.'</BaseImponible>
                         '.$strimpu.'
                         <MontoTotalLinea>'.$total.'</MontoTotalLinea>
                   </LineaDetalle>
                </DetalleServicio>
        ...
        function generaxmlNCV44($pventa){
...
';
        ...
        $xml.='
                ...
                <InformacionReferencia>
                        <TipoDocIR>'.$tipref.'</TipoDocIR>
                        <Numero>'.$pventa->clavefe.'</Numero>
                        <FechaEmisionIR>'.$fechao.'</FechaEmisionIR>
                        <Codigo>01</Codigo>
                        <Razon>Anula</Razon>
                </InformacionReferencia>
                </NotaCreditoElectronica>';
...
function generaxmlTiqueteV44($pventa){
            ...
            ';
                ...
                if($pventa->tipocedfa!='00'){
                        $xml.='<Receptor>
                                ...
                                <CorreoElectronico>'.$pventa->emailremi.'</CorreoElectronico>
                                </Receptor>';
        }
